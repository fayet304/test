...
async function loadKaryawanDropdown() {
  ...
  try {
    ...
    select.innerHTML = '<option value="" disabled selected>Gagal memuat daftar karyawan</option>';
  }
}
...
async function loadPengaturanPublik() {
  try {
    ...
    durasiSelect.innerHTML = '<option value="" disabled selected>Pilih durasi</option>';
    (res.data.durasiIzin || []).forEach(d => {
      const opt = document.createElement('option');
...
function initFormAbsensi() {
  const namaSelect = document.getElementById('input-nama');
  ...
  const izinWrapper = document.getElementById('izin-wrapper');
  ...
  btnVerify.addEventListener('click', async () => {
    const id = namaSelect.value;
    const pin = pinInput.value.trim();
    if (!id || !pin) return;
    btnVerify.disabled = true;
    btnVerify.textContent = '...';
    try {
      const res = await Api.post({ action: 'verifyPin', id, pin });
      if (res.success && res.data.valid) {
        State.verifiedKaryawan = { id, nama: res.data.nama };
  ...
  document.querySelectorAll('.jenis-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      izinWrapper.classList.toggle('hidden', State.jenisTerpilih !== 'Izin Keluar');
      lokasiWrapper.classList.remove('hidden');
      updateSubmitState();
    });
  });

  document.getElementById('input-durasi').addEventListener('change', updateSubmitState);
  document.getElementById('input-alasan').addEventListener('input', updateSubmitState);
  ...
  form.addEventListener('submit', async e => {
    ...
    if (State.jenisTerpilih === 'Izin Keluar') {
      const durasiRaw = document.getElementById('input-durasi').value;
      if (!durasiRaw) return;
      const durasi = JSON.parse(durasiRaw);
      payload.durasiLabel = durasi.label;
      payload.durasiMenit = durasi.menit;
      payload.alasan = document.getElementById('input-alasan').value.trim();
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        btnSubmit.textContent = 'Kirim Absensi';
      }
    } catch (err) {
  ...
  function resetFormAfterNama() {
    ...
    document.querySelectorAll('.jenis-btn').forEach(b => b.classList.remove('active'));
    document.getElementById('input-durasi').value = '';
  ...
  function updateSubmitState() {
    let ready = !!State.verifiedKaryawan && !!State.jenisTerpilih;
    if (State.jenisTerpilih === 'Izin Keluar') {
      const durasiOk = !!document.getElementById('input-durasi').value;
      const alasanOk = document.getElementById('input-alasan').value.trim() !== '';
      ready = ready && durasiOk && alasanOk;
    }
...
function jenisBadge(jenis) {
  const map = { Masuk: 'badge-masuk', Pulang: 'badge-pulang', 'Izin Keluar': 'badge-izin' };
